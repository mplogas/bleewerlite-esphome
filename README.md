...
- **Connection lockup on ungraceful disconnect.** Some Neewer lights lock up if the BLE connection drops without a proper GATT disconnect (e.g. power loss, hard crash). The light refuses new connections until power-cycled. The component includes a shutdown handler that sends a graceful disconnect on OTA updates and reboots, which prevents this in most cases. Hard power loss to the ESP32 is the one scenario where this can still happen. If your light stops responding, power-cycle it. (Thanks to [u/entropy512](https://www.reddit.com/user/entropy512/) for [flagging this](https://github.com/Entropy512/rgb_led_filmscan/blob/main/neewer_light.py).)
